...
if(!function_exists('libero_mikado_styles')) {
    ...
    function libero_mikado_styles() {
        wp_register_style('libero-mikado-blog', MIKADO_ASSETS_ROOT.'/css/blog.min.css');

        //include theme's core styles
        wp_enqueue_style('libero-mikado-default-style', MIKADO_ROOT.'/style.css');
        wp_enqueue_style('libero-mikado-modules-plugins', MIKADO_ASSETS_ROOT.'/css/plugins.min.css');
        wp_enqueue_style('libero-mikado-modules', MIKADO_ASSETS_ROOT.'/css/modules.min.css');

        //Print-specific CSS
        wp_enqueue_style("libero-mikado-print", MIKADO_ASSETS_ROOT . "/css/print.min.css");

        libero_mikado_icon_collections()->enqueueStyles();

        if(libero_mikado_load_blog_assets()) {
            wp_enqueue_style('libero-mikado-blog');
        }
        ...
        if(libero_mikado_load_woo_assets()) {
            $style_dynamic_deps_array[] = 'libero-mikado-woocommerce';
        }

        //is responsive option turned on?
        if(libero_mikado_is_responsive_on()) {
            wp_enqueue_style('libero-mikado-modules-responsive', MIKADO_ASSETS_ROOT.'/css/modules-responsive.min.css');
            wp_enqueue_style('libero-mikado-blog-responsive', MIKADO_ASSETS_ROOT.'/css/blog-responsive.min.css');
            if(libero_mikado_load_woo_assets()) {
                $style_dynamic_deps_array[] = 'libero-mikado-woocommerce-responsive';
            }
            ...
            if(file_exists(MIKADO_ROOT_DIR.'/assets/css/style_dynamic_responsive.css') && libero_mikado_is_css_folder_writable() && !is_multisite()) {
                wp_enqueue_style('libero-mikado-style-dynamic-responsive', MIKADO_ASSETS_ROOT.'/css/style_dynamic_responsive.css', array(), filemtime(MIKADO_ROOT_DIR.'/assets/css/style_dynamic_responsive.css'));
            } else {
                wp_enqueue_style('libero-mikado-style-dynamic-responsive', MIKADO_ASSETS_ROOT.'/css/style_dynamic_responsive.php');
            }
        }

        if(file_exists(MIKADO_ROOT_DIR.'/assets/css/style_dynamic.css') && libero_mikado_is_css_folder_writable() && !is_multisite()) {
            wp_enqueue_style('libero-mikado-style-dynamic', MIKADO_ASSETS_ROOT.'/css/style_dynamic.css', $style_dynamic_deps_array, filemtime(MIKADO_ROOT_DIR.'/assets/css/style_dynamic.css')); //it must be included after woocommerce styles so it can override it
        } else {
            wp_enqueue_style('libero-mikado-style-dynamic', MIKADO_ASSETS_ROOT.'/css/style_dynamic.php', $style_dynamic_deps_array); //it must be included after woocommerce styles so it can override it
        }
...
if(!function_exists('libero_mikado_google_fonts_styles')) {
    ...
    function libero_mikado_google_fonts_styles() {
        ...
        if (count($fonts_array) > 0) {
            ...
            wp_enqueue_style( 'libero-mikado-google-fonts', esc_url_raw($libero_mikado_fonts), array(), '1.0.0' );
...
if(!function_exists('libero_mikado_scripts')) {
    ...
    function libero_mikado_scripts() {
	    ...
	    wp_enqueue_script( 'easypiechart', MIKADO_ASSETS_ROOT . '/js/modules/plugins/easypiechart.min.js', array( 'jquery' ), false, true );
	    ...
	    wp_enqueue_script( 'counter-random', MIKADO_ASSETS_ROOT . '/js/modules/plugins/counter-random.min.js', array( 'jquery' ), false, true );
        ...
        wp_enqueue_script('libero-mikado-modules', MIKADO_ASSETS_ROOT.'/js/modules.min.js', array('jquery'), false, true);

        if(libero_mikado_load_blog_assets()) {
            wp_enqueue_script('libero-mikado-blog', MIKADO_ASSETS_ROOT.'/js/blog.min.js', array('jquery'), false, true);
        }
...
if ( ! function_exists( 'libero_mikado_enqueue_editor_customizer_styles' ) ) {
	/**
	 * Enqueue supplemental block editor styles
	 */
	function libero_mikado_enqueue_editor_customizer_styles() {
		wp_enqueue_style( 'libero-mikado-editor-blocks-styles', MIKADO_FRAMEWORK_ADMIN_ASSETS_ROOT . '/css/editor-blocks-style.css' );
		wp_enqueue_style( 'libero-mikado-editor-customizer-styles', MIKADO_FRAMEWORK_ADMIN_ASSETS_ROOT . '/css/editor-customizer-style.css' );
	}
	
	// add google font
	add_action( 'enqueue_block_editor_assets', 'libero_mikado_google_fonts_styles' );
	// add action
	add_action( 'enqueue_block_editor_assets', 'libero_mikado_enqueue_editor_customizer_styles' );
}
...
if( !function_exists('libero_mikado_sidebar_layout') ) {
    ...
    function libero_mikado_sidebar_layout() {
	    ...
	    if ( ! empty( $sidebar_layout ) && ! is_active_sidebar( libero_mikado_get_sidebar() ) ) {
		    $sidebar_layout = '';
	    }
...
if( !function_exists('libero_mikado_page_custom_style') ) {
    ...
    function libero_mikado_page_custom_style() {
       ...
       if($style !== '') {
	       $html .= $style;
	       wp_add_inline_style( 'libero-mikado-modules', $html);
       }
    }

   add_action('wp_enqueue_scripts', 'libero_mikado_page_custom_style');
}
...
if(!function_exists('libero_mikado_print_custom_css')) {
    ...
    function libero_mikado_print_custom_css() {
        ...
        if($custom_css !== '') {
            wp_add_inline_style( 'libero-mikado-modules', $custom_css);
        }
    }
...
if(!function_exists('libero_mikado_print_custom_js')) {
    ...
    function libero_mikado_print_custom_js() {
	    ...
	    if($custom_js !== '') {
		    wp_add_inline_script('libero-mikado-modules', $custom_js);
	    }
    }
	
	add_action('wp_enqueue_scripts', 'libero_mikado_print_custom_js');
}
...
if(!function_exists('libero_mikado_get_global_variables')) {
    ...
    function libero_mikado_get_global_variables() {
        ...
        wp_localize_script('libero-mikado-modules', 'mkdGlobalVars', array(
...
if(!function_exists('libero_mikado_per_page_js_variables')) {
    function libero_mikado_per_page_js_variables() {
        ...
        wp_localize_script('libero-mikado-modules', 'mkdPerPageVars', array(
...
if (!function_exists('libero_mikado_is_gutenberg_editor')) {
	/**
	 * Function that checks if gutenberg is installed
	 * @return bool
	 */
	function libero_mikado_is_gutenberg_editor() {
		return class_exists( 'WP_Block_Type' );
	}
}

if (!function_exists('libero_mikado_is_gutenberg_plugin')) {
	/**
	 * Function that checks if gutenberg is installed
	 * @return bool
	 */
	function libero_mikado_is_gutenberg_plugin() {
...
if ( ! function_exists( 'libero_mikado_get_module_part' ) ) {
	function libero_mikado_get_module_part( $module ) {
		return $module;
	}
}