<?php
/*
   Class: LiberoFramework
   A class that initializes Mkd Framework
*/
class LiberoFramework {
    ...
    private function __construct() {
        $this->mkdOptions = LiberoOptions::get_instance();
        $this->mkdMetaBoxes = LiberoMetaBoxes::get_instance();
    }
	...
	public function setSkin(LiberoSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class LiberoSkinManager
 *
    ...
    private function setSkin($skinName = 'mikado') {
        if($skinName !== '') {
            if(file_exists(get_template_directory().'/framework/admin/skins/'.$skinName.'/skin.php')) {
	            ...
	            $this->skin = new LiberoMikadoSkin();
            }
        } else {
            $this->skin = false;
        }
...
/**
 * Class LiberoSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class LiberoSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if($template !== '') {
            include(libero_mikado_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
        }
    }
    ...
    public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo libero_mikado_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
/*
   Class: LiberoOptions
   A class that initializes Mkd Options
*/
class LiberoOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $libero_mikado_options)) {
            return $libero_mikado_options[$key];
...
/*
   Class: LiberoAdminPage
   A class that initializes Mkd Admin Page
*/
class LiberoAdminPage implements iLiberoLayoutNode {
    ...
    function __construct($slug="", $title_label="", $icon = "") {
        $this->layout = array();
        $this->factory = new LiberoFieldFactory();
        $this->slug = $slug;
    ...
    public function renderChild(iLiberoRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: LiberoMetaBoxes
   A class that initializes Mkd Meta Boxes
*/
class LiberoMetaBoxes {
...
/*
   Class: LiberoMetaBox
   A class that initializes Mkd Meta Box
*/
class LiberoMetaBox implements iLiberoLayoutNode {
	...
	public function setTitle($label) {
		...
		return $libero_mikado_Framework->getSkin()->getSkinLabel().' '.$label;
 	}
}
