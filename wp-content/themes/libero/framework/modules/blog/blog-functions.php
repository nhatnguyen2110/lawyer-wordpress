				...
				echo '<ul>';
					...
					for ($i=1; $i <= $pages; $i++){
						if (1 != $pages &&( !($i >= $paged+$range+1 || $i <= $paged-$range-1) || $pages <= $showitems )){
							echo (libero_mikado_get_module_part($paged == $i))? "<li class='active'><span>".$i."</span></li>":"<li><a href='".get_pagenum_link($i)."' class='inactive'>".$i."</a></li>";
						}
					}
...
if (!function_exists('libero_mikado_modify_read_more_link')) {
	...
	function libero_mikado_modify_read_more_link() {
		...
		if(libero_mikado_core_installed()) {
			$link .= libero_mikado_get_button_html(
				array(
...
if(!function_exists('libero_mikado_read_more_button')) {
	...
	function libero_mikado_read_more_button($option = '', $class = 'mkd-blog-read-more') {
		...
		if($show_read_more_button && !libero_mikado_post_has_read_more() && !post_password_required() && libero_mikado_core_installed()) {
