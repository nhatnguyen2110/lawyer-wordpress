...
if($video == 'show_video'){
	...
	if($v_image) {
		$video_output .= '<img ' . libero_mikado_get_inline_attr($v_image, 'src') . ' width="1920" height="800" title="'. esc_attr__('No video playback capabilities','libero') .'" alt="'. esc_attr__('video thumb','libero') .'" />';
	}
	$video_output .='</object>';
	$video_output .='</video>';
	$video_output .='</div>';
...
print libero_mikado_get_module_part($output);